...
class Osu():
    ...
    async def osu(self, name):
        ...
        await self.bot.say(
            "```xl\n{}'s profile  \n-------------- \nRank: #{} (#{} {}) \nPP: {} | Level: {} \nTotal Score: {} \nAcc: {}``` \n<https://osu.ppy.sh/u/{}> \nhttps://a.ppy.sh/{}".format(
                name, profile[0].pp_rank, profile[0].pp_country_rank, profile[0].country, profile[0].pp_raw,
                profile[0].level, profile[0].total_score, profile[0].accuracy, profile[0].user_id, profile[0].user_id))
